    ...
    .action((file) => {
        fs.stat(file, (err, stats) => {
            ...
            const options = {
                ...
                json: true,
                headers: {
                    'content-length': stats.size,// this option is IMPORTANT to define as we are streaming the file to the server rather than handling all the content to the request all at once - important for HTTP protocol regarding timing to close or not the socket connection!!!
                    'content-type': 'application/json'
                }
            };

            const req = request.post(options);// the req object is a writable and readable stream so we can pipe in and out

            const stream = fs.createReadStream(file);
            stream.pipe(req);// we are pipping in the stream from the file - writable
